...
module.exports = {
  ...
  sub
    .setName("delete")
    .setDescription("Delete reaction role by message ID.")
    .addStringOption(o =>
      o
        .setName("message_id")
        .setDescription("Message ID of the reaction role")
        .setRequired(true)
    )
),
...
async execute(interaction) {
  ...
  if (sub === "add-existing") {
    ...
    return interaction.editReply(
      "✅ Reaction role added to existing message."
    );
  }

  if (sub === "list") {
    const data =
      await ReactionRole.find({
        guildId: interaction.guild.id
      }).sort({
        createdAt: -1
      });
  ...
  if (sub === "delete") {
    ...
    if (!reactionRole) {
      ...
      return interaction.editReply(
        "❌ No reaction role found with this message ID."
      );
    ...
    return interaction.editReply(
      "✅ Reaction role deleted successfully."
    );
  }
}
